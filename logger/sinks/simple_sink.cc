...
SimpleSink::SimpleSink(detail::Conf conf) : conf_(std::move(conf)){
    INFO("SimpleSink: file_dir={}, file_name={}, single_size={}",
...
void SimpleSink::Log(const LogMsg& msg) {
...
void SimpleSink::SetFormatter(std::unique_ptr<Formatter> formatter) {
    formatter_ = std::move(formatter);
}

void SimpleSink::Flush() {
    TIMER_COUNT("SimpleSink::Flush");
    CacheToFile_();
}

void SimpleSink::WriteToCache_(const void* data, size_t size) {
...
bool SimpleSink::NeedWriteToFile_() {
    std::lock_guard<std::mutex> lock(mtx_);
    return cache_.GetRatio() > 0.8;
}

void SimpleSink::CacheToFile_() {
    std::lock_guard<std::mutex> lock(mtx_);
    TIMER_COUNT("SimpleSink::CacheToFile_");
...
std::filesystem::path SimpleSink::GetFilePath_() {
    constexpr char kSinkFlag[] = "_simple";
    if (file_path_.empty()) {
